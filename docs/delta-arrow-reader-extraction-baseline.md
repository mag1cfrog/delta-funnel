...
| `DeltaSourceConfig` | `delta_funnel_owned_integration` | Remains `delta_funnel::DeltaSourceConfig`; owns the source name plus Delta Funnel's pre-load environment and authentication policy, then constructs `delta_arrow_reader::DeltaTableBuilder`. | Rust loading, session registry, Python | #474 boundary tests; #475 API docs | [rustdoc](https://docs.rs/delta-funnel/0.5.0/delta_funnel/struct.DeltaSourceConfig.html) |
...
| `RegisteredDeltaSource` | `delta_funnel_owned_integration` | Remains `delta_funnel::RegisteredDeltaSource`; owns the sanitized URI, schema, and product protocol report while registration delegates to `delta_arrow_reader::register_delta_table`. | Session registry, reports, tests | #474 registration shadow tests; #475 migration docs | [rustdoc](https://docs.rs/delta-funnel/0.5.0/delta_funnel/struct.RegisteredDeltaSource.html) |
