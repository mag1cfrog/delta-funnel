...
[Choose a Delta Funnel Profiling Method](profiling.md). For the capture
lifecycle and health-field interpretation, use
[Record and Inspect a Unified Perfetto Trace](profiling-perfetto.md). This
report does not replace those guides.
...
For the exact commands and repository checks behind this evidence, see
[Reproduce the Profiling Validation](profiling-validation-reproduction.md).
