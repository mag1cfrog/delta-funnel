...
/// the classifier already proved retained filters only reference partition
/// columns.
///
...
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
enum PartitionScalarAdapterError {
    UnsupportedArrowType,
    UnsupportedScalarValue,
}

fn arrow_partition_type_to_kernel_primitive(
    ...
    match data_type {
        ...
        ArrowDataType::Decimal128(precision, scale) => {
            KernelPrimitiveType::decimal(*precision, u8::try_from(*scale).ok()?).ok()
        }
        _ => None,
    }
}

fn kernel_partition_scalar_to_datafusion_scalar(
    scalar: Scalar,
    data_type: &ArrowDataType,
) -> Result<ScalarValue, PartitionScalarAdapterError> {
    match (scalar, data_type) {
        ...
        // Kernel partition parsing does not preserve signed zero or non-finite spellings.
        ...
        // An empty Delta partition string denotes null, not empty binary data.
        ...
        // Preserve the provider Arrow type only when precision and scale match exactly.
        ...
        _ => Err(PartitionScalarAdapterError::UnsupportedScalarValue),
    }
}
...
/// literal true placeholder. Literal false is not a
...
mod tests {
    ...
    -> Result<(), Box<dyn std::error::Error>> {
        ...
        assert_eq!(
            kernel_partition_scalar_to_datafusion_scalar(
                ...
                &ArrowDataType::Timestamp(ArrowTimeUnit::Microsecond, Some(Arc::clone(&timezone)),),
            ),
            Ok(ScalarValue::TimestampMicrosecond(
    ...
    #[test]
    fn partition_scalar_adapter_rejects_unproven_values() -> Result<(), Box<dyn std::error::Error>>
    {
        let unsupported = PartitionScalarAdapterError::UnsupportedScalarValue;
    ...
    #[test]
    fn evaluation_failure_keeps_file() -> Result<(), String> {
        let (dynamic, retained) = retained_filter(vec![column("region", 1)])?;
        dynamic
            .update(column("region", 99))
            .map_err(|err| err.to_string())?;
